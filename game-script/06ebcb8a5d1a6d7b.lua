...
local function parsePetName(itemName)
    ...
    if species and weightStr then
        species = string.gsub(species, "^%s*(.-)%s*$", "%1")
...
local function getToolWeight(tool)
    local weightAttr = tool:GetAttribute("Weight") or tool:GetAttribute("w") or tool:GetAttribute("weight") or tool:GetAttribute("HatchWeight") or tool:GetAttribute("hatchWeight")
    if weightAttr and type(weightAttr) == "number" then
        return weightAttr
    end
    local weightStr = string.match(tool.Name, "[%[%(]%s*([%d%.]+)%s*kg%s*[%]%)]") or string.match(tool.Name, "([%d%.]+)%s*kg")
    if weightStr then
    ...
    for _, child in ipairs(tool:GetChildren()) do
        if child:IsA("NumberValue") or child:IsA("StringValue") then
            ...
            if string.find(lowerName, "weight") or lowerName == "w" then
...
local function getPetTypesList()
    local defaultTypes = {"Any", "Orange Tabby", "Rooster", "Pig", "Cow", "Sheep", "Chicken", "Bunny", "Goat", "Cat", "Dog", "Axolotl", "Duck", "Frog", "Bee", "Turtle"}
    local seen = {}
    local list = {}
    ...
    if bp then
        for _, tool in ipairs(bp:GetChildren()) do
            if tool:IsA("Tool") and tool:GetAttribute("PET_UUID") then
                ...
                local species = parsePetName(name)
...
local function autoListPets()
    local hasBooth, myBooth = playerHasBooth()
    if not hasBooth or not myBooth then return end
    ...
    local listedCount = 0
    for _, tool in ipairs(bp:GetChildren()) do
        if listedCount >= slots then break end
        if tool:IsA("Tool") and tool:GetAttribute("PET_UUID") then
            ...
            if listedItemNames[name] then continue end
            
            local species, weight = parsePetName(name)
            if not weight then
                weight = getToolWeight(tool)
            end
            
            if config.filterPetType ~= "Any" and string.lower(species) ~= string.lower(config.filterPetType) then
                continue
            end
            
            local minW = tonumber(config.filterMinWeight)
            if minW and (not weight or weight < minW) then
                continue
            end
            
            local maxW = tonumber(config.filterMaxWeight)
            if maxW and (not weight or weight > maxW) then
                continue
            end
            
            local price = tonumber(config.filterPrice)
            if not price or price <= 0 then
                continue
            end
            
            local ok, _ = safeInvoke(CreateListingInvoke, "Pet", petUUID, price)
            if ok then
                notify("Auto Listed Pet!", name .. " @ 🪙 " .. formatPrice(price), "success")
                listedItemNames[name] = true
...
sectionHdr(settingsPage,"Auto Pet Listing",4.5)
local petToggleRow, getPetToggle, setPetToggle = mkToggle(settingsPage, "Enable Auto List Pets", config.autoListPets, 4.6, function(v)
    config.autoListPets = v
    saveConfig()
end)

local filterCard = mkCard(settingsPage, 112, 4.7)
pad(filterCard, 8, 10, 8, 10)

local fLayout = Instance.new("UIListLayout", filterCard)
fLayout.SortOrder = Enum.SortOrder.LayoutOrder
fLayout.Padding = UDim.new(0, 6)

local dropdownRow = Instance.new("Frame", filterCard)
dropdownRow.Size = UDim2.new(1, 0, 0, 28)
dropdownRow.BackgroundTransparency = 1
dropdownRow.LayoutOrder = 1

local dLabel = Instance.new("TextLabel", dropdownRow)
dLabel.Size = UDim2.new(0.4, 0, 1, 0)
dLabel.BackgroundTransparency = 1
dLabel.Text = "Target Pet Type:"
dLabel.TextColor3 = T.TEXT2
dLabel.Font = Enum.Font.GothamBold
dLabel.TextSize = 11
dLabel.TextXAlignment = Enum.TextXAlignment.Left

local dropdownBtn = mkBtn(dropdownRow, "Pet: " .. (config.filterPetType or "Any"), T.ACCENT_DIM, 150, 28)
dropdownBtn.Position = UDim2.new(1, -150, 0, 0)
dropdownBtn.TextSize = 11

local dropdownMenu = Instance.new("ScrollingFrame", settingsPage)
dropdownMenu.Name = "FilterDropdownMenu"
dropdownMenu.Size = UDim2.new(0, 150, 0, 120)
dropdownMenu.Position = UDim2.new(0.5, 50, 0.45, 0)
dropdownMenu.BackgroundColor3 = T.BG_CARD2
dropdownMenu.ZIndex = 30
dropdownMenu.Visible = false
dropdownMenu.ScrollBarThickness = 3
dropdownMenu.ScrollBarImageColor3 = T.ACCENT
corner(dropdownMenu, 6)
stroke(dropdownMenu, T.BORDER_LIT)

local dMenuLayout = Instance.new("UIListLayout", dropdownMenu)
dMenuLayout.Padding = UDim.new(0, 3)

dMenuLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    dropdownMenu.CanvasSize = UDim2.new(0, 0, 0, dMenuLayout.AbsoluteContentSize.Y + 6)
end)
...
local function populateDropdown()
    ...
    for _, t in ipairs(types) do
        ...
        btn.MouseEnter:Connect(function() btn.BackgroundColor3 = T.BG_HOVER end)
        btn.MouseLeave:Connect(function() btn.BackgroundColor3 = T.BG_CARD end)
        
        btn.MouseButton1Click:Connect(function()
            config.filterPetType = t
            dropdownBtn.Text = "Pet: " .. t
            dropdownMenu.Visible = false
            saveConfig()
        end)
    end
...
local weightRow = Instance.new("Frame", filterCard)
weightRow.Size = UDim2.new(1, 0, 0, 28)
weightRow.BackgroundTransparency = 1
weightRow.LayoutOrder = 2

local wLabel = Instance.new("TextLabel", weightRow)
wLabel.Size = UDim2.new(0.4, 0, 1, 0)
wLabel.BackgroundTransparency = 1
wLabel.Text = "Weight Range (kg):"
wLabel.TextColor3 = T.TEXT2
wLabel.Font = Enum.Font.GothamBold
wLabel.TextSize = 11
wLabel.TextXAlignment = Enum.TextXAlignment.Left

local minBox = mkTextBox(weightRow, "Min Wt")
minBox.Size = UDim2.new(0, 70, 0, 26)
minBox.Position = UDim2.new(1, -150, 0, 1)
minBox.Text = config.filterMinWeight or ""

local toLabel = Instance.new("TextLabel", weightRow)
toLabel.Size = UDim2.new(0, 10, 1, 0)
toLabel.Position = UDim2.new(1, -78, 0, 0)
toLabel.BackgroundTransparency = 1
toLabel.Text = "-"
toLabel.TextColor3 = T.TEXT3
toLabel.Font = Enum.Font.GothamBold
toLabel.TextSize = 12

local maxBox = mkTextBox(weightRow, "Max Wt")
maxBox.Size = UDim2.new(0, 70, 0, 26)
maxBox.Position = UDim2.new(1, -70, 0, 1)
maxBox.Text = config.filterMaxWeight or ""

minBox:GetPropertyChangedSignal("Text"):Connect(function()
    config.filterMinWeight = minBox.Text
    saveConfig()
end)

maxBox:GetPropertyChangedSignal("Text"):Connect(function()
    config.filterMaxWeight = maxBox.Text
    saveConfig()
end)

local priceRow = Instance.new("Frame", filterCard)
priceRow.Size = UDim2.new(1, 0, 0, 28)
priceRow.BackgroundTransparency = 1
priceRow.LayoutOrder = 3

local pLabel = Instance.new("TextLabel", priceRow)
pLabel.Size = UDim2.new(0.4, 0, 1, 0)
pLabel.BackgroundTransparency = 1
pLabel.Text = "Listing Price:"
pLabel.TextColor3 = T.TEXT2
pLabel.Font = Enum.Font.GothamBold
pLabel.TextSize = 11
pLabel.TextXAlignment = Enum.TextXAlignment.Left

local pBox = mkTextBox(priceRow, "Price (Tokens)")
pBox.Size = UDim2.new(0, 150, 0, 26)
pBox.Position = UDim2.new(1, -150, 0, 1)
pBox.Text = config.filterPrice or ""

pBox:GetPropertyChangedSignal("Text"):Connect(function()
    config.filterPrice = pBox.Text
    saveConfig()
end)
