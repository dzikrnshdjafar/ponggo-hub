...
#   by: Wardz | versi: 3.2 (+ N-Package Auto Grid + Auto-Detect)
...
# Multi package count — jumlah package Roblox yang di-grid bersama (1-8)
PKG_COUNT=1
...
save_config() {
    ...
    # Package count — arg 13, default baca dari file jika ada
    local pkg_count="${13}"
    # Backward compat: baca AUTO_GRID lama kalau PKG_COUNT tidak ada
    if [ -f "$cfg_file" ]; then
        ...
        if [ -z "$pkg_count" ]; then
            local old_grid
            old_grid=$(grep '^AUTO_GRID=' "$cfg_file" | head -1 | cut -d= -f2)
            if [ "$old_grid" = "1" ]; then
                # Migrate old AUTO_GRID=1 to PKG_COUNT=2
                pkg_count=2
            else
                pkg_count=$(grep '^PKG_COUNT=' "$cfg_file" | head -1 | cut -d= -f2)
            fi
        fi
    fi
    t_wait="${t_wait:-120}"
...
# Package count for Auto Grid (0/1=single, 2=dual, 3+=multi)
PKG_COUNT=$pkg_count
...
persist_discord_settings() {
    local cfg_file=$1
    local pkg=$2
    local saved_url saved_mode saved_relog saved_reconnect saved_restart saved_home saved_error_code saved_pkg_count
    ...
    if [ -f "$cfg_file" ]; then
        ...
        saved_pkg_count=$(grep '^PKG_COUNT=' "$cfg_file" | head -1 | cut -d= -f2)
        if [ -z "$saved_pkg_count" ]; then
            local old_grid
            old_grid=$(grep '^AUTO_GRID=' "$cfg_file" | head -1 | cut -d= -f2)
            [ "$old_grid" = "1" ] && saved_pkg_count=2 || saved_pkg_count=1
        fi
    fi

    save_config "$cfg_file" "$pkg" "$saved_url" "$saved_mode" \
        "$saved_relog" "$saved_reconnect" "$saved_restart" "$saved_home" \
        "${saved_error_code:-1}" "" "" "" "${saved_pkg_count:-1}"
}
...
# ── Auto-populate PKG1..PKG{N} from installed packages ────────
auto_detect_and_set_packages() {
    local detected=()
    ...
    if [ ${#detected[@]} -eq 0 ]; then
        PKG1="${PKG1:-com.roblox.client}"
        PKG2=""; PKG3=""; PKG4=""; PKG5=""; PKG6=""; PKG7=""; PKG8=""
        PKG_COUNT=1
        return
    fi
    ...
    local count=0
    local i=0
    while [ $i -lt ${#sorted[@]} ] && [ $count -lt 8 ]; do
        local n=$((count+1))
        eval "PKG$n=\"${sorted[$i]}\""
        count=$((count+1))
        i=$((i+1))
    ...
    # Set PKG_COUNT to the number of detected packages
...
wizard_setup_pkg() {
    ...
    # Save — PKG_COUNT ditentukan di level main, bukan per-package
    local cfg_file="${CONFIG_BASE_DIR}/roblox_config_${pkg}.cfg"
    save_config "$cfg_file" "$pkg" "$url" "$mode" "$relog" "$reconnect" "$restart" "$home" "$error_code" "" "" "" "$PKG_COUNT"
...
menu_ganti_url_mode_pkg() {
    ...
    local keep_relog keep_reconnect keep_restart keep_home keep_error_code keep_pkg_count
    ...
    keep_pkg_count=$(grep '^PKG_COUNT=' "$cfg_file" | head -1 | cut -d= -f2)
    if [ -z "$keep_pkg_count" ]; then
        local old_grid
        old_grid=$(grep '^AUTO_GRID=' "$cfg_file" | head -1 | cut -d= -f2)
        [ "$old_grid" = "1" ] && keep_pkg_count=2 || keep_pkg_count=1
    fi
    ...
    save_config "$cfg_file" "$pkg" "$new_url" "$new_mode" \
        ...
        "${keep_error_code:-1}" "" "" "" "$keep_pkg_count"
...
menu_edit_settings_pkg() {
    ...
    while true; do
        local cur_url cur_mode cur_relog cur_reconnect cur_restart cur_home cur_error_code cur_pkg_count
        ...
        cur_pkg_count=$(grep '^PKG_COUNT=' "$cfg_file" | head -1 | cut -d= -f2)
        if [ -z "$cur_pkg_count" ]; then
            local old_grid
            old_grid=$(grep '^AUTO_GRID=' "$cfg_file" | head -1 | cut -d= -f2)
            [ "$old_grid" = "1" ] && cur_pkg_count=2 || cur_pkg_count=1
        fi
        ...
        show_current_config "$cur_url" "$cur_mode" "$cur_relog" "$cur_reconnect" "$cur_restart" "$cur_home" "$cur_error_code" "$cur_pkg_count"
        ...
        echo "  6) Package Count         (sekarang: $cur_pkg_count)"
        ...
        case $PILIHAN in
            1)
                ...
                read -r V
                if [[ "$V" =~ ^[0-9]+$ ]]; then
                    save_config "$cfg_file" "$pkg" "$cur_url" "$cur_mode" "$V" "$cur_reconnect" "$cur_restart" "$cur_home" "$cur_error_code" "" "" "" "$cur_pkg_count"
                    echo "  ✅ Disimpan!"
                else
                    echo "  ⚠ Masukkan angka!"
                fi
            ...
            2)
                ...
                save_config "$cfg_file" "$pkg" "$cur_url" "$cur_mode" "$cur_relog" "$new_val" "$cur_restart" "$cur_home" "$cur_error_code" "" "" "" "$cur_pkg_count"
                echo "  ✅ Reconnect: $(show_toggle $new_val)"
                sleep 1
                ;;
            3)
                local new_val; new_val=$([ "$cur_restart" = "1" ] && echo 0 || echo 1)
                save_config "$cfg_file" "$pkg" "$cur_url" "$cur_mode" "$cur_relog" "$cur_reconnect" "$new_val" "$cur_home" "$cur_error_code" "" "" "" "$cur_pkg_count"
                echo "  ✅ Restart: $(show_toggle $new_val)"
                sleep 1
                ;;
            4)
                local new_val; new_val=$([ "$cur_home" = "1" ] && echo 0 || echo 1)
                save_config "$cfg_file" "$pkg" "$cur_url" "$cur_mode" "$cur_relog" "$cur_reconnect" "$cur_restart" "$new_val" "$cur_error_code" "" "" "" "$cur_pkg_count"
                echo "  ✅ Home RC: $(show_toggle $new_val)"
                sleep 1
                ;;
             5)
                local new_val; new_val=$([ "$cur_error_code" = "1" ] && echo 0 || echo 1)
                save_config "$cfg_file" "$pkg" "$cur_url" "$cur_mode" "$cur_relog" "$cur_reconnect" "$cur_restart" "$cur_home" "$new_val" "" "" "" "$cur_pkg_count"
            ...
            6)
                # ── Package Count ─────────────────────────────────────────
                local active_count
                active_count=$(get_active_pkg_count)
                echo ""
                echo "  Jumlah package yang di-grid bersama? (1-8)"
                echo "  (Terdeteksi: $active_count)"
                printf "  > "
                read -r V
                if [[ "$V" =~ ^[0-9]+$ ]] && [ "$V" -ge 1 ] && [ "$V" -le 8 ]; then
                    cur_pkg_count="$V"
                    save_config "$cfg_file" "$pkg" "$cur_url" "$cur_mode" "$cur_relog" "$cur_reconnect" "$cur_restart" "$cur_home" "$cur_error_code" "$cur_wait" "$cur_verify" "$cur_watchdog" "$cur_pkg_count"
                    PKG_COUNT="$V"
                    echo "  ✅ Package Count: $V"
                else
                    echo "  ⚠ Masukkan angka 1-8"
                fi
                sleep 1
                ;;
            7)
                        ...
                        1)
                            ...
                            if [[ "$V" =~ ^[0-9]+$ ]] && [ "$V" -ge 30 ] && [ "$V" -le 600 ]; then
                                ...
                                save_config "$cfg_file" "$pkg" "$cur_url" "$cur_mode" \
                                    ...
                                    "$cur_error_code" "$cur_wait" "$cur_verify" "$cur_watchdog" "$cur_pkg_count"
...
# ── Helper: count non-empty PKG vars ─────────────────────────
get_active_pkg_count() {
    local cnt=0 i=1
    while [ $i -le 8 ]; do
        local var="PKG$i"
        local val
...
setup_or_load_pkg() {
    ...
    while true; do
        ...
        show_current_config "$saved_url" "$saved_mode" "$saved_relog" "$saved_reconnect" "$saved_restart" "$saved_home" "$saved_error_code" "$saved_pkg_count"
...
auto_grid_arrange() {
    log "🔲 Auto Grid: arranging $PKG_COUNT windows..."
    ...
    while [ $i -le 8 ]; do
        ...
        if [ -n "$pkg" ]; then
            pkgs+=("$pkg")
            # Read URL from per-package config
            local cfg="${CONFIG_BASE_DIR}/roblox_config_${pkg}.cfg"
            local mode url
            if [ -f "$cfg" ]; then
                mode=$(grep '^MODE=' "$cfg" | head -1 | cut -d'"' -f2)
                url=$(grep '^URL=' "$cfg" | head -1 | cut -d'"' -f2)
            fi
            mode="${mode:-market}"
            url="${url:-$URL_MARKET}"
            local active_url
            ...
            local act
            act=$(get_view_activity "$pkg" "$active_url")
            acts+=("$act")
            count=$((count+1))
        fi
        i=$((i+1))
    ...
    [ "$count" -lt 2 ] && log "⚠️ Auto Grid: <2 packages — arranging anyway" && count=$PKG_COUNT

    # Grid calculation: cols = ceil(sqrt(count))
    local cols=1
    ...
    [ "$rows" -lt 1 ] && rows=1
    local cell_w=$(( disp_w / cols ))
    local cell_h=$(( disp_h / rows ))
    log "🔲 Grid: ${cols}×${rows}, cell: ${cell_w}×${cell_h}"

    # Launch & arrange each package
    local idx=0
    while [ $idx -lt $count ]; do
        ...
        local x=$(( col * cell_w ))
        local y=$(( row * cell_h ))
        local w=$(( (col + 1) * cell_w ))
        local h=$(( (row + 1) * cell_h ))
        # Expand last row/col to fill remaining space
        [ $((col + 1)) -eq $cols ] && w=$disp_w
        [ $((row + 1)) -eq $rows ] && h=$disp_h

        log "🔲 Package $((idx+1)) ($pkg) → cell ($col,$row) ${x},${y} ${w}×${h}"

        # Force-stop then launch fresh in freeform
        am force-stop "$pkg" 2>/dev/null
        sleep 1
        am start -a android.intent.action.VIEW -d "$act" -n "$act" \
            --windowingMode 5 -f 0x10000000 </dev/null >/dev/null 2>&1
        sleep 4

        # Resize to grid cell
        local tid
        tid=$(get_task_id_for_pkg "$pkg")
        if [ -n "$tid" ]; then
            am task resize "$tid" "$x" "$y" "$w" "$h" 2>/dev/null
            if [ $? -eq 0 ]; then
                log "✅ Auto Grid: $pkg resized to cell ($col,$row)"
            else
                log "⚠️ Auto Grid: $pkg resize failed"
            fi
        else
            log "⚠️ Auto Grid: cannot get task ID for $pkg"
        fi
        idx=$((idx+1))
    done

    log "✅ Auto Grid done — $count packages arranged"
    send_discord_notification "floating" "Auto Grid — $count packages arranged" "${pkgs[0]:-Roblox}"
}
    ...
    while true; do
        ...
        if [ "$app_alive" = "0" ]; then
            ...
            if [ "$miss_count" -ge 3 ]; then
                ...
                # Kill budget check — cegah infinite restart loop
                if ! check_kill_budget "$pkg"; then
                    release_crash_lock "$pkg"
                    sleep 10
                    continue
                fi

                # Auto-setor cookie sebelum rejoin
                auto_setor_cookie "$pkg"

                # Setup keep-alive untuk package
                setup_keepalive "$pkg"
...
# ─────────────────────────────────────────
#   COOKIE MANAGEMENT (dari zenx_worker.lua)
# ─────────────────────────────────────────

# Extract .ROBLOSECURITY cookie dari SQLite database Roblox
# Path: /data/data/<pkg>/app_webview/Default/Cookies
extract_cookie() {
    local pkg=$1
    ...
    # Copy ke /tmp supaya tidak lock database yang dipakai Roblox
    local tmp_db="/data/local/tmp/rbx_cookies_${pkg}.db"
    ...
    # Query cookie ROBLOSECURITY dari SQLite
    local cookie
    cookie=$(sqlite3 "$tmp_db" \
        "SELECT encrypted_value FROM cookies WHERE host_key='.roblox.com' AND name='.ROBLOSECURITY' LIMIT 1;" \
        2>/dev/null)

    rm -f "$tmp_db" 2>/dev/null

    # Cookie Android di-encrypt; kalau encrypted_value ada, kita perlu decrypt
    # Untuk sekarang, coba path alternatif: /data/data/<pkg>/app_webview/Default/Local\ Storage
    # atau grep langsung dari binary cookie file
    if [ -z "$cookie" ]; then
        # Fallback: cari di binary cookie file
        local cookie_file="/data/data/$pkg/app_webview/Default/Cookies"
        cookie=$(strings "$cookie_file" 2>/dev/null | grep -oE "WARNING:-DO-NOT-SHARE--THIS--IS--A--SHAREABLE--.ROBLOSECURITY--.*" | head -1)
    fi

    echo "$cookie"
}

# Validate cookie via Roblox API
# Returns: alive / dead / captcha / ban / unknown
validate_cookie() {
    local cookie=$1
    ...
    # Check for ban
    if echo "$resp" | grep -qiE "ban|terminat|moderat"; then
        echo "ban"
        return
    fi

    # Check for captcha
    if echo "$resp" | grep -qiE "captcha|challenge"; then
        echo "captcha"
        return
    fi

    # Check for valid response (has name field)
    local username
    username=$(echo "$resp" | grep -oE '"name":"[^"]*"' | head -1 | cut -d'"' -f4)
    if [ -n "$username" ]; then
        echo "alive"
        return
    fi
    ...
    if echo "$resp" | grep -qiE "Unauthorized|401"; then
        echo "dead"
        return
    fi
...
# Get username from cookie (decode from .ROBLOSECURITY value)
get_username_from_cookie() {
    ...
    # Roblox cookie format: _|WARNING:-DO-NOT-SHARE-...--<base64_data>
    # Base64 decode the part after the last underscore
    local b64
    b64=$(echo "$cookie" | grep -oE "[A-Za-z0-9+/=]{20,}$" | tail -1)
    if [ -n "$b64" ]; then
        local decoded
        decoded=$(echo "$b64" | base64 -d 2>/dev/null | strings 2>/dev/null | head -1)
        if [ -n "$decoded" ]; then
            echo "$decoded"
            return
        fi
    ...
    # Fallback: get from prefs.xml
    local pkg=$2
    if [ -n "$pkg" ]; then
        local prefs="/data/data/$pkg/shared_prefs/com.roblox.player.player.xml"
        if [ -f "$prefs" ]; then
            grep -oE '<string name="LastUserName">[^<]*</string>' "$prefs" 2>/dev/null \
                | cut -d'>' -f2 | cut -d'<' -f1
        fi
    fi
}

# Inject cookie ke client (SQLite UPDATE)
# PERHATIAN: Roblox harus DIHENTI dulu sebelum inject
inject_cookie() {
    local pkg=$1
    local cookie=$2
    local db_path="/data/data/$pkg/app_webview/Default/Cookies"
    ...
    if [ ! -f "$db_path" ]; then
        log "❌ Cookie inject gagal: database tidak ditemukan untuk $pkg"
        return 1
    fi

    if [ -z "$cookie" ]; then
        log "❌ Cookie inject gagal: cookie kosong"
        return 1
    fi

    # Force-stop dulu supaya database tidak dikunci
    am force-stop "$pkg" 2>/dev/null
    sleep 2

    # Copy database ke /tmp untuk edit
    local tmp_db="/data/local/tmp/rbx_cookies_inject.db"
    cp "$db_path" "$tmp_db" 2>/dev/null

    # Update cookie
    sqlite3 "$tmp_db" \
        "UPDATE cookies SET value='$cookie' WHERE host_key='.roblox.com' AND name='.ROBLOSECURITY';" \
        2>/dev/null

    # Copy balik
    cp "$tmp_db" "$db_path" 2>/dev/null
    chmod 600 "$db_path" 2>/dev/null
    chown "$(stat -c '%u:%g' "$db_path" 2>/dev/null || echo '0:0')" "$db_path" 2>/dev/null

    rm -f "$tmp_db" 2>/dev/null
    log "✅ Cookie injected ke $pkg"
    return 0
}

# Auto-setor cookie ke panel (jika panel dikonfigurasi)
auto_setor_cookie() {
    local pkg=$1
    local cookie
    ...
    local username
    username=$(get_username_from_cookie "$cookie" "$pkg")

    # Validate
    local status
    status=$(validate_cookie "$cookie")

    # Kirim ke panel jika ada
    if [ -n "$PANEL_URL" ] && [ -n "$PANEL_KEY" ]; then
        curl -s -m 10 -X POST \
            -H "X-Kunci: $PANEL_KEY" \
            -H "Content-Type: application/json" \
            -d "{\"akun\":\"$username\",\"paket\":\"$pkg\",\"cookie\":\"$cookie\"}" \
            "$PANEL_URL/cookie-simpan" >/dev/null 2>&1

        curl -s -m 10 -X POST \
            -H "X-Kunci: $PANEL_KEY" \
            -H "Content-Type: application/json" \
            -d "{\"akun\":\"$username\",\"status\":\"$status\"}" \
            "$PANEL_URL/cookie-status" >/dev/null 2>&1
    fi

    log "🍪 Cookie: $username ($status) — $pkg"
}

# ─────────────────────────────────────────
#   KILL BUDGET (max restarts per window)
# ─────────────────────────────────────────
# Mencegah infinite restart loop — jika crash terlalu sering,
# stop auto-restart dan tunggu intervensi manual.

KILL_BUDGET_MAX=5           # maks restart per window
KILL_BUDGET_WINDOW=1800     # window waktu (30 menit)

# File tracking: /data/local/tmp/rbx_killcount_<pkg>
get_kill_count() {
    local pkg=$1
    local state_dir="${STATE_BASE_DIR}/rbx_state_${pkg}"
    local count_file="$state_dir/kill_count"
    local window_file="$state_dir/kill_window_start"
    ...
    if [ $((now - window_start)) -gt "$KILL_BUDGET_WINDOW" ]; then
        echo "0"
        echo "0" > "$count_file"
...
check_kill_budget() {
    local pkg=$1
    local count
    count=$(get_kill_count "$pkg")

    if [ "$count" -ge "$KILL_BUDGET_MAX" ]; then
        log "⛔ KILL BUDGET EXCEEDED — $pkg telah di-restart $count kali dalam $((KILL_BUDGET_WINDOW/60)) menit"
        log "⛔ Auto-restart DIHENTikan — tunggu $((KILL_BUDGET_WINDOW/60)) menit atau restart manual"
        return 1
    fi
    return 0
}
...
# ─────────────────────────────────────────
#   CAPTCHA DETECTION (via logcat)
# ─────────────────────────────────────────
# Deteksi FunCaptcha/arkose dari logcat — Roblox menampilkan
# captcha saat login dari IP baru atau cookie expired.

captcha_detected=0

check_captcha_logcat() {
    local pkg=$1
    local logcat_out
    logcat_out=$(logcat -d -b main -T "$(date '+%m-%d %H:%M:%S.000')" 2>/dev/null | grep -i "$pkg" | tail -20)

    if echo "$logcat_out" | grep -qiE "captcha|arkose|funcaptcha|challenge.*verify"; then
        captcha_detected=1
        log "🤖 CAPTCHA DETECTED — $pkg membutuhkan verifikasi manual"
        update_dashboard_state "$pkg" "captcha" "Captcha detected"
        send_discord_notification "captcha" "FunCaptcha/arkose detected — needs manual solve" "$pkg"
        return 0
    fi
    return 1
}
...
# ─────────────────────────────────────────
#   LANDSCAPE LOCK + KEEP-ALIVE
# ─────────────────────────────────────────
# Lock landscape mode supaya semua window freeform orientasi sama
# + deviceidle whitelist + oom_score_adj supaya Android tidak kill proses

lock_landscape() {
    local changed=0
    # Lock landscape (rotation 1 = landscape)
    if [ "$(settings get system accelerometer_rotation 2>/dev/null)" != "0" ]; then
        settings put system accelerometer_rotation 0 2>/dev/null && changed=1
    fi
    if [ "$(settings get system user_rotation 2>/dev/null)" != "1" ]; then
        settings put system user_rotation 1 2>/dev/null && changed=1
    fi
    if [ "$changed" = "1" ]; then
...
setup_keepalive() {
    ...
    # deviceidle whitelist — cegah Doze mode kill Roblox
    local my_id
    my_id=$(id -u 2>/dev/null || echo "0")
    if command -v deviceidle >/dev/null 2>&1; then
        deviceidle whitelist +"$pkg" 2>/dev/null
    elif [ -f "/system/bin/deviceidle" ]; then
        /system/bin/deviceidle whitelist +"$pkg" 2>/dev/null
    fi

    # oom_score_adj — hint ke Android bahwa proses ini penting
    local pid
    pid=$(get_pid_for_pkg "$pkg")
    if [ -n "$pid" ] && [ -f "/proc/$pid/oom_score_adj" ]; then
        echo "-300" > "/proc/$pid/oom_score_adj" 2>/dev/null
        log "🔧 oom_score_adj set to -300 for $pkg (PID $pid)"
    fi

    # appops — izinkan background activity
...
echo "========================================="
echo ""
echo "  Mau auto-detect Roblox packages?"
echo "  (Scan semua package com.roblox.clien* di device)"
echo ""
echo "  1) Auto-detect (rekomendasi)"
echo "  2) Manual setup"
echo ""
...
if [ "$SETUP_CHOICE" = "2" ]; then
    echo ""
    echo "  Jumlah package yang akan di-grid? (1-8)"
    printf "  > "
    read -r PKG_COUNT
    if ! [[ "$PKG_COUNT" =~ ^[0-9]+$ ]] || [ "$PKG_COUNT" -lt 1 ] || [ "$PKG_COUNT" -gt 8 ]; then
        PKG_COUNT=1
    fi

    pkg_num=1
    while [ $pkg_num -le "$PKG_COUNT" ]; do
        echo ""
        pilih_package "📦 PILIH PACKAGE $pkg_num" "PKG$pkg_num"
        set_pkg_paths "$(eval echo \$PKG$pkg_num)" "PKG$pkg_num"
        check_clone_app "$(eval echo \$PKG$pkg_num)"
        setup_or_load_pkg "$(eval echo \$PKG$pkg_num)" "$pkg_num"
        pkg_num=$((pkg_num+1))
    done
else
    # Auto-detect mode
    ...
    if [ "$PKG_COUNT" -ge 1 ]; then
        echo ""
        echo "  ✅ Terdeteksi $PKG_COUNT package:"
        pkg_num=1
        while [ $pkg_num -le "$PKG_COUNT" ]; do
            echo "     $pkg_num) $(eval echo \$PKG$pkg_num)"
            pkg_num=$((pkg_num+1))
        done
        echo ""
        echo "  Edit jumlah jika perlu? (1-8, atau Enter untuk lanjut)"
        echo "  (Misal: ketik 3 kalau hanya mau 3 dari 5 yang terdeteksi)"
        printf "  > "
        read -r EDIT_COUNT
        if [[ "$EDIT_COUNT" =~ ^[0-9]+$ ]] && [ "$EDIT_COUNT" -ge 1 ] && [ "$EDIT_COUNT" -le 8 ]; then
            PKG_COUNT="$EDIT_COUNT"
            # Clear unused slots
            n=$((PKG_COUNT+1))
            while [ $n -le 8 ]; do
                eval "PKG$n=\"\""
        ...
        while [ $pkg_num -le "$PKG_COUNT" ]; do
            cur_pkg=
            eval cur_pkg="\$PKG$pkg_num"
            if [ -n "$cur_pkg" ]; then
                set_pkg_paths "$cur_pkg" "PKG$pkg_num"
                check_clone_app "$cur_pkg"
                setup_or_load_pkg "$cur_pkg" "$pkg_num"
            fi
            pkg_num=$((pkg_num+1))
        done
    else
        echo "  ⚠ Tidak ada package terdeteksi. Setup manual."
        PKG_COUNT=1
        echo ""
...
# Simpan setting Discord ke config semua package
local di=1
while [ $di -le "$PKG_COUNT" ]; do
    dpkg=
    eval dpkg="\$PKG$di"
    [ -n "$dpkg" ] && persist_discord_settings "${CONFIG_BASE_DIR}/roblox_config_${dpkg}.cfg" "$dpkg"
    di=$((di+1))
done

# Load config — baca PKG_COUNT dari file
source "${CONFIG_BASE_DIR}/roblox_config_${PKG1}.cfg" 2>/dev/null
saved_count=
saved_count=$(grep '^PKG_COUNT=' "${CONFIG_BASE_DIR}/roblox_config_${PKG1}.cfg" 2>/dev/null | head -1 | cut -d= -f2)
if [ -z "$saved_count" ]; then
    old_grid=
    old_grid=$(grep '^AUTO_GRID=' "${CONFIG_BASE_DIR}/roblox_config_${PKG1}.cfg" 2>/dev/null | head -1 | cut -d= -f2)
    [ "$old_grid" = "1" ] && saved_count=2 || saved_count=1
fi
PKG_COUNT="${saved_count:-$PKG_COUNT}"
...
echo "=========================================" | tee -a "$PKG1_LOG_FILE"
log "Package Count    : $PKG_COUNT"
li=1
while [ $li -le "$PKG_COUNT" ]; do
    lpkg=
    eval lpkg="\$PKG$li"
    [ -n "$lpkg" ] && log "  Package $li     : $lpkg"
    li=$((li+1))
done
...
# Open extra packages if enabled
...
# Launch semua monitor untuk semua package
mi=1
while [ $mi -le "$PKG_COUNT" ]; do
    mpkg=
    eval mpkg="\$PKG$mi"
    if [ -n "$mpkg" ]; then
        mcfg="${CONFIG_BASE_DIR}/roblox_config_${mpkg}.cfg"
        _mon_start "monitor_${mi}"  monitor_events         "$mpkg" "$mcfg"
        _mon_start "crash_${mi}"    crash_monitor          "$mpkg" "$mcfg"
        _mon_start "logcat_${mi}"   logcat_crash_detector  "$mpkg" "$mcfg"
        _mon_start "errcode_${mi}"  error_code_monitor     "$mpkg" "$mcfg"
        _mon_start "watchdog_${mi}" stuck_watchdog         "$mpkg" "$mcfg"
        _mon_start "ocr_${mi}"      ocr_error_monitor      "$mpkg" "$mcfg"
    fi
    mi=$((mi+1))
done
...
(
    ...
    while true; do
        ki=1
        while [ $ki -le "$PKG_COUNT" ]; do
            kpkg=
            eval kpkg="\$PKG$ki"
            if [ -n "$kpkg" ]; then
                kcfg="${CONFIG_BASE_DIR}/roblox_config_${kpkg}.cfg"
                _mon_alive "monitor_${ki}"  || { log "⚠️ monitor_${ki} mati — restart";        _mon_start "monitor_${ki}"  monitor_events        "$kpkg" "$kcfg"; }
                _mon_alive "crash_${ki}"    || { log "⚠️ crash_${ki} mati — restart";          _mon_start "crash_${ki}"    crash_monitor         "$kpkg" "$kcfg"; }
                _mon_alive "logcat_${ki}"   || { log "⚠️ logcat_${ki} mati — restart";         _mon_start "logcat_${ki}"   logcat_crash_detector "$kpkg" "$kcfg"; }
                _mon_alive "errcode_${ki}"  || { log "⚠️ errcode_${ki} mati — restart";        _mon_start "errcode_${ki}"  error_code_monitor    "$kpkg" "$kcfg"; }
                _mon_alive "watchdog_${ki}" || { log "⚠️ watchdog_${ki} mati — restart";       _mon_start "watchdog_${ki}" stuck_watchdog        "$kpkg" "$kcfg"; }
                _mon_alive "ocr_${ki}"      || { log "⚠️ ocr_${ki} mati — restart";            _mon_start "ocr_${ki}"      ocr_error_monitor     "$kpkg" "$kcfg"; }
            fi
            ki=$((ki+1))
        done
...
# Dashboard di foreground — menampilkan semua package
sleep 1
if command -v python >/dev/null 2>&1; then
    ...
    while [ $di -le "$PKG_COUNT" ]; do
        dpkg=
        eval dpkg="\$PKG$di"
        [ -n "$dpkg" ] && dash_pkgs="$dash_pkgs $dpkg"
        di=$((di+1))
    done
    log "📊 Buka dashboard: python ${DASH_DIR}/dashboard.py $dash_pkgs"
    python ${DASH_DIR}/dashboard.py $dash_pkgs
